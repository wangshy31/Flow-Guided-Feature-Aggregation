...
class MutableModule(BaseModule):
    ...
    def fit(self, train_data, eval_data=None, eval_metric='acc',
        ...
        tmp_mem_cell = mx.nd.zeros((4, 2048, 94, 94), ctx = mx.gpu())
        tmp_mem_hidden = mx.nd.zeros((4, 2048, 94, 94), ctx = mx.gpu())
        for epoch in range(begin_epoch, num_epoch):
            ...
            for nbatch, data_batch in enumerate(train_data):
                ...
                for index in range(pre_filename.shape[0]):
                    ...
                    mx.nd.expand_dims(tmp_mem_cell[index], axis=0).copyto(data_batch.data[index][5])
                    mx.nd.expand_dims(tmp_mem_hidden[index], axis=0).copyto(data_batch.data[index][6])
                ...
                for index in range(pre_filename.shape[0]):
                    ...
                    tmp_mem_cell[index,:, 0:shape2, 0:shape3] = self.get_outputs(merge_multi_context = False)[5][index]
                    ...
                    tmp_mem_hidden[index,:, 0:shape2, 0:shape3] = self.get_outputs(merge_multi_context = False)[6][index]
