...
class resnet_v1_101_flownet_rfcn(Symbol):
    ...
    def get_rpn_symbol(self, cfg):
        ...
        if cfg.TEST.CXX_PROPOSAL:
            ...
            rois = mx.sym.Custom(
                ...
                rpn_pre_nms_top_n=cfg.TEST.RPN_PRE_NMS_TOP_N, rpn_post_nms_top_n=cfg.TEST.RPN_POST_NMS_TOP_N,
                threshold=cfg.TEST.RPN_NMS_THRESH, rpn_min_size=cfg.TEST.RPN_MIN_SIZE)
        group = mx.sym.Group([data, aggregated_conv_feat, rois, gt_roi_cache, flow_grid])
        self.sym = group
        return group
    ...
    def get_rcnn_symbol(self, cfg):
        ...
        #print 'shape b', b
        #psroipooled_cls_rois_mean = mx.contrib.sym.PSROIPooling(name='psroipooled_cls_rois', data=rfcn_cls_single[cfg.TEST.KEY_FRAME_INTERVAL], rois=gt_roi_cache,
                                                           #group_size=7, pooled_size=7,
                                                           #output_dim=num_classes, spatial_scale=0.0625)
        psroipooled_loc_rois_mean = mx.contrib.sym.PSROIPooling(name='psroipooled_loc_rois', data=rfcn_bbox_single[cfg.TEST.KEY_FRAME_INTERVAL],
                                                                rois=gt_roi_cache,
                                                                group_size=7, pooled_size=7,
                                                                output_dim=8, spatial_scale=0.0625)
