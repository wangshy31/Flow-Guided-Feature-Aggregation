...
class resnet_v1_101_flownet_rfcn(Symbol):
    ...
    def get_train_scoreflow_symbol(self, cfg):
        ...
        conv_feats = mx.sym.SliceChannel(conv_feat_split[0], axis=1, num_outputs=2)
        conv_feats_bef = mx.sym.SliceChannel(conv_feat_split[1], axis=1, num_outputs=2)
        conv_feats_aft = mx.sym.SliceChannel(conv_feat_split[2], axis=1, num_outputs=2)
        ...
        # res5
        rfcn_feat = mx.symbol.Concat(*[conv_feats[1], conv_feats_bef[1], conv_feats_aft[1]], dim=0)
        ...
        scores_bbox = mx.sym.SliceChannel(rfcn_bbox, axis=0, num_outputs=3)
        ...
        warp_score_box_1 = mx.sym.BilinearSampler(data=scores_bbox[1], grid=flow_grid_1, name='warping_score_box_1')
        warp_score_box_2 = mx.sym.BilinearSampler(data=scores_bbox[2], grid=flow_grid_2, name='warping_socre_box_2')
        ...
        rfcn_loc_mean = scores_bbox[0]+warp_score_box_1+warp_score_box_1
        rfcn_loc_mean = rfcn_loc_mean/3.0
        #rfcn_bbox = mx.sym.Convolution(data=rfcn_feat, kernel=(1, 1), num_filter=7 * 7 * 4 * num_reg_classes,
        ...
        psroipooled_loc_rois = mx.contrib.sym.PSROIPooling(name='psroipooled_loc_rois', data=rfcn_loc_mean, rois=rois,
                                                           group_size=7,
                                                           pooled_size=7,
    ...
    def get_scoreflow_aggregation_symbol(self, cfg):
        # config alias for convenient
        num_classes = cfg.dataset.NUM_CLASSES
        num_reg_classes = (2 if cfg.CLASS_AGNOSTIC else num_classes)
        num_anchors = cfg.network.NUM_ANCHORS
        ...
        #rfcn_feat = conv_feats[1]
        #rfcn_feat = mx.symbol.Concat(*[conv_feats[1], bef_conv_feats[1], aft_conv_feats[1]], dim=0)
        conv_feat_cache = mx.sym.SliceChannel(conv_feat, axis=1, num_outputs=2)
        rfcn_cls = mx.sym.Convolution(data=conv_feat_cache[1], kernel=(1, 1), num_filter=7 * 7 * num_classes, name="rfcn_cls")
        ...
        warp_scores = mx.sym.BilinearSampler(data=rfcn_cls, grid=flow_grid, name='warping_scores')
        scores = mx.sym.SliceChannel(warp_scores, axis=0, num_outputs=data_range)
        aggregated_conv_score = 0
        for i in range(data_range):
            aggregated_conv_score += scores[i]
        rfcn_cls_mean = aggregated_conv_score/data_range
    ...
    def get_aggregation_symbol(self, cfg):
        ...
        aux_feat_cache = mx.symbol.slice_axis(feat_cache, axis=1, begin=0, end=1024)
        aux_conv_feat = mx.symbol.Concat(mx.symbol.slice_axis(aux_feat_cache, axis=0, begin=0, end=cfg.TEST.KEY_FRAME_INTERVAL),
                                          aggregated_conv_feat,
                                          mx.symbol.slice_axis(aux_feat_cache, axis=0, begin=cfg.TEST.KEY_FRAME_INTERVAL+1, end=data_range),
                                          dim=0)
        aux_conv_feats = mx.sym.SliceChannel(aux_conv_feat, axis=1, num_outputs=2)
        ...
        #rfcn_feat = conv_feats[1]
        #rfcn_feat = mx.symbol.Concat(*[conv_feats[1], bef_conv_feats[1], aft_conv_feats[1]], dim=0)
        rfcn_cls = mx.sym.Convolution(data=aux_conv_feats[1], kernel=(1, 1), num_filter=7 * 7 * num_classes, name="rfcn_cls")
        warp_scores = mx.sym.BilinearSampler(data=rfcn_cls, grid=flow_grid, name='warping_scores')
        scores = mx.sym.SliceChannel(warp_scores, axis=0, num_outputs=data_range)
        aggregated_conv_score = 0
        for i in range(data_range):
