...
class resnet_v1_101_flownet_rfcn(Symbol):
    ...
    def get_correlation_train_symbol(self, cfg):
        ...
        correlation_1 = mx.symbol.Correlation(data1=conv_feat[0], data2=conv_feat[1], kernel_size=1,
                                              max_displacement=8, pad_size=8, name='correlation_1')
        correlation_2 = mx.symbol.Correlation(data1=conv_feat[0], data2=conv_feat[2], kernel_size=1,
                                              max_displacement=8, pad_size=8, name='correlation_2')
        correlation_1_pool = mx.sym.Pooling(name='correlation_1_pool', data=correlation_1, pool_type='avg',
                                   global_pool=True,
                                   kernel=(100,100))
        correlation_2_pool = mx.sym.Pooling(name='correlation_2_pool', data=correlation_2, pool_type='avg',
                                   global_pool=True,
                                   kernel=(100,100))
        correlation_1_weights = mx.symbol.softmax(data=correlation_1_pool, axis=1)
        correlation_2_weights = mx.symbol.softmax(data=correlation_2_pool, axis=1)

        weighted_correlation_1 = mx.symbol.broadcast_mul(correlation_1, correlation_1_weights, name='weighted_correlation_1')
        weighted_correlation_2 = mx.symbol.broadcast_mul(correlation_2, correlation_2_weights, name='weighted_correlation_2')

        sum_weight1 = mx.symbol.sum(weighted_correlation_1, axis=1, name='weight1')
        sum_weight2 = mx.symbol.sum(weighted_correlation_2, axis=1, name='weight2')

        sigmoid_weight1 = mx.symbol.Activation(name='sigmoid_weight1', data=sum_weight1, act_type='sigmoid')
        sigmoid_weight2 = mx.symbol.Activation(name='sigmoid_weight2', data=sum_weight2, act_type='sigmoid')
        all_weight = sigmoid_weight1+sigmoid_weight2
        select_conv_feat = conv_feat[0] + mx.symbol.broadcast_mul(conv_feat[0],all_weight)
    ...
    def get_correlation_test_symbol(self, cfg):
        # config alias for convenient
        num_classes = cfg.dataset.NUM_CLASSES
        num_reg_classes = (2 if cfg.CLASS_AGNOSTIC else num_classes)
        num_anchors = cfg.network.NUM_ANCHORS
        ...
        cur_data = mx.symbol.slice_axis(conv_feat, axis=0, begin=cfg.TEST.KEY_FRAME_INTERVAL, end=cfg.TEST.KEY_FRAME_INTERVAL+1)
        cur_data_copies = mx.sym.tile(cur_data, reps=(data_range, 1, 1, 1))
        correlation = mx.symbol.Correlation(data1=cur_data_copies, data2=conv_feat, kernel_size=1,
                                              max_displacement=8, pad_size=8, name='correlation')
        correlation_pool = mx.sym.Pooling(name='correlation_pool', data=correlation, pool_type='avg',
                                   global_pool=True,
                                   kernel=(100,100))
        correlation_weights = mx.symbol.softmax(data=correlation_pool, axis=1)

        weighted_correlation = mx.symbol.broadcast_mul(correlation, correlation_weights, name='weighted_correlation')

        sum_weight = mx.symbol.sum(weighted_correlation, axis=1, name='sum_weight')

        sigmoid_weight = mx.symbol.Activation(name='sigmoid_weight', data=sum_weight, act_type='sigmoid')
        all_weight =  mx.symbol.mean(sigmoid_weight, axis = 0, name='all_weight')
        select_conv_feat = mx.symbol.broadcast_mul(cur_data, all_weight, name='select_conv_feat')
    ...
    def get_feat_symbol(self, cfg):
        ...
        embed_feat = mx.sym.Concat(conv_feat,conv_feat, dim=1)
