...
class resnet_v1_101_flownet_rfcn(Symbol):
    ...
    def get_train_symbol(self, cfg):
        ...
        conv_feat = mx.sym.SliceChannel(mem_ReLU1, axis=0, num_outputs=3)
        ...
        #memory
        mem_hidden = mx.symbol.zeros_like(conv_feat[0], name='mem_hidden')
        mem_cell = mx.symbol.zeros_like(conv_feat[0], name='mem_cell')
        #features for bef
        mem_bef_cell, mem_bef_hidden = self.get_lstm_symbol(conv_feat[1], mem_cell, mem_hidden)
        #features for data
        ...
        mem_add_data = mem_data_hidden + conv_feat[0]
    ...
    def get_mem_aver_symbol(self, cfg):
        ...
        #features for aft
        mem_bef_warp = mx.sym.BilinearSampler(data=conv_feat[1], grid=flow_grid_1, name='mem_bef_warp')
        mem_aft_add = mem_bef_warp + conv_feat[2]
        mem_aft_aver = mem_aft_add / 2
        #features for data
        mem_aft_warp = mx.sym.BilinearSampler(data=mem_aft_aver, grid=flow_grid_2, name='mem_aft_warp')
        mem_data_add = mem_aft_warp + conv_feat[0]
        mem_data_aver = mem_data_add /2

        conv_feats = mx.sym.SliceChannel(mem_data_aver, axis=1, num_outputs=2)
